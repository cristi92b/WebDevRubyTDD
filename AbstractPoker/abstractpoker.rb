...
def classify_poker_hand(poker_hand)
	...
	c1=poker_hand.select{|i|}.uniq.size

	if(t1==1 && !(poker_hand.include?(min) && poker_hand.include?(min+1) && poker_hand.include?(min+2) && poker_hand.include?(min+3) && poker_hand.include?(min+4)))
		return :high_card
